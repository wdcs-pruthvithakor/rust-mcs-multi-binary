...
use futures::StreamExt; // StreamExt is used for Stream methods like `next()`
...
pub async fn aggregator_process(num_clients: usize, public_keys: Arc<Vec<VerifyingKey>>) {
    ...
    // let (tx, rx) = mpsc::channel::<bool>(num_clients);
    ...
    while let Ok((stream, _)) = listener.accept().await {
        ...
        tokio::spawn(async move {
            ...
            // Split the WebSocket stream into a read and write half
            // let (mut read, _) = ws_stream.split();

            // Read messages from the WebSocket stream
            while let Some(msg) = ws_stream.next().await {
                // Use read.next(), not split_sink.next()
                match msg {
                    ...
                    Ok(Message::Text(text)) => {
                        // if let Err(e) = process_message(&text, &public_keys).await {
                        //     eprintln!("Error processing message: {}", e);
                        // }
                        match process_message(&text, &public_keys).await {
    ...
    // clients_verified += 1;
    // averages.push(avg);
    // let mut avg_vec = averages.lock().await;
    // let averages_copy = avg_vec.clone(); // Clone the data
    // avg_vec.clear();
    // if let Some(global_avg) = utils::calculate_average(&averages_copy) {
    //     println!("Aggregator: Global average BTC price: {:.4}", global_avg);
    //     utils::save_global_data(&averages_copy, global_avg)
...
//     // println!("{message}");
//     // Convert Vec<u8> to [u8; 64]
//     let signature_vec = general_purpose::STANDARD.decode(data["signature"].as_str().unwrap())?;
//     let signature_array: &[u8; 64] = signature_vec
//         .as_slice()
//         .try_into()
//         .map_err(|_| "Signature length is not 64 bytes")?;
